...
uses
  ...
  FireDAC.Stan.Param, FireDAC.DatS, FireDAC.DApt.Intf, FireDAC.DApt, System.IniFiles, System.IOUtils,
...
type
  TFormBaseConnetion = class(TDataModule)
    ...
    procedure QryCadastroTarefasAfterInsert(DataSet: TDataSet);
    procedure QryCadastroTarefasAfterEdit(DataSet: TDataSet);
  private
  ...
  public
    ...
    function GetByIdUsuarios(const AId: Integer): Boolean;
...
begin
 inherited Create(AOwner); // Always call the inherited first in the constructor
...
begin
  ...
  try
    ...
    Result := TJOSE.SHA256CompactToken(GetJWTPrivateKeyFromIni,LJWT);
  finally
    LJWT.Free;
  end;
end;

function TFormBaseConnetion.GetByIdUsuarios(const AId: Integer): Boolean;
begin
...
procedure TFormBaseConnetion.QryCadastroTarefasAfterEdit(DataSet: TDataSet);
begin
  QryCadastroTarefasatualizado_em.AsDateTime := Now;
end;

procedure TFormBaseConnetion.QryCadastroTarefasAfterInsert(DataSet: TDataSet);
begin
  QryCadastroTarefascriado_em.AsDateTime := Now;
  QryCadastroTarefasstatus.AsString      := 'PENDENTE'; 
end;
